...
from utils.const import UINT8_MAX
from utils.nn_utils import get_dataloader
...
def get_tensors(data_dpath, is_trainval):
    mnist = datasets.MNIST(data_dpath, train=is_trainval, download=True)
...
def get_data(hparams):
    ...
    torch.manual_seed(0)
    x_trainval, df_trainval = get_tensors(hparams.data_dpath, True)
    x_test, df_test = get_tensors(hparams.data_dpath, False)
    ...
    data_train = get_dataloader(dataset_train, hparams.batch_size, hparams.workers, True, False)
    data_val = get_dataloader(dataset_val, hparams.batch_size, hparams.workers, False, True)
    data_test = get_dataloader(dataset_test, hparams.batch_size, hparams.workers, False, False)
